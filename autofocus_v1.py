...
class Autofocus:
    ...
    class ImagingConditions:
        def __init__(self, autofocus, res, hfw, dwell, reduced_area=None):
    ...
    def laplacian_fit(self, wds, iqs):
        ...
        wd_range = np.linspace(min(wds), max(wds), 200)
    ...
    def optimize_wd(self, bounds):
        ...
        print(f"  [Stage 1a] Large HFW = {self.hfw_large * 1e3:.3f} mm, {self.n_points_LS_LHFW} points ...")
        ...
        print(f"  [Stage 1b] Small HFW = {self.hfw_small * 1e3:.3f} mm, {self.n_points_LS_SHFW} points ...")
        ...
        if CI_HR_LHFW < alpha_LR_LHFW and CI_HR_SHFW < alpha_LR_SHFW:
            print(" Warning the signal for converging is low on both large and small HFWs.")

            # # Stage 2: quick large-HFW coarse search
            # print(f"\n[Stage 2] Coarse scan (low res, large HFW = {self.hfw_large * 1e3:.3f} mm) "
            ...
            # print(f"\n[Stage 2] Coarse scan (low res, small HFW = {self.hfw_small * 1e3:.3f} mm) "
        ...
        if not self.do_gaussian_fit:
            ...
            if A < 0:
                print("  Warning: Gaussian fit amplitude is negative. Using converged WD.")
                return wd_optimized
