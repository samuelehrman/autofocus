...
class Autofocus:
    ...
    def convergent_search(self, bounds, max_iterations=15, tolerance=0.001):
        ...
        lbd = math.isclose(result.x, lower_bound, rel_tol = .05 * lower_bound)
        upd = math.isclose(result.x, upper_bound, rel_tol= .05 * upper_bound)
        count = 0

        while (lbd or upd) and count <= 1:
            print(f'Warning: Optimal WD is on the boundary of search inputs. Expanding search.')
            ...
            if lbd:
                ...
                print(f'Convergent search in [{bounds[0]*1e3:.4f}, {bounds[1]*1e3:.4f}] mm{bounds}')
        ...
        if peak >= 1.5*bg:
            ...
            print(f'Sharpness peak is not 1.5 times background. Image clarity may be unsatisfactory.')
